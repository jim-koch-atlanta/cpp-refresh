...
5. Implement my own **allocator**.
  * Do a ring buffer... that could be interesting and fun.
6. Update my BST to use the ring buffer allocator.
7. Performance testing with ring buffer allocator vs with default allocator. Number of allocs / deallocs. Maybe some sort of actual performance metrics (Wall clock? CPU clock?)
  * Benchmark harness -- `<chrono> stead_clock` for wall time, and reuse TrackingAllocator to count allocs/deallocs.
  * Compare my allocator vs. `std::prm::monotonic_buffer_resource`.
8. In-order iterator.
...
11. operator<< as a hidden friend
  * Also make the member variables of tree and node private.
12. Deduction guide + initialize_list ctor
